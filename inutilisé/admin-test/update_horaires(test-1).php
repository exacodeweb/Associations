...
include '../admin/db_connect.php';//db_connect.php
...
<form method="post" action="../admin/update_hours(test-1).php">
...
<?php
include('../admin/csrf_token_hours.php'); // Vérifiez que le chemin est correct
...
// Inclure CSRF Token si nécessaire
if (file_exists('../php/csrf_token.php')) {
...
// Vérifier les données POST et les traiter
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $jour = $_POST['jour'] ?? '';
    $ouverture = $_POST['ouverture'] ?? '';
    $fermeture = $_POST['fermeture'] ?? '';
    ...
    try {
        ...
        $conn = new PDO("mysql:host=" . DB_HOST . ";dbname=site_papillons" . DB_NAME, DB_USER, DB_PASSWORD);
