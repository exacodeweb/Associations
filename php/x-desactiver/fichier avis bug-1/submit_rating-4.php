<?php
session_name('session_usersrating_avis')
if (session_status('avis-session') == PHP_SESSION_NONE) {
    session_start();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO userratings (nom, email, message, note) VALUES (?, ?, ?, ?)");
...
<!--?php
session_start();
require_once('../config/config-4.php'); // Assurez-vous que ce fichier contient la connexion à la base de données
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO avis (nom, email, message, note) VALUES (?, ?, ?, ?)");
    $stmt->bind_param('sssi', $nom, $email, $message, $note);
